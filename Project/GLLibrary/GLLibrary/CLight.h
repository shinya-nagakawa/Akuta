...
public:
	...
	**/
	...
	// スポットライトの角度設定
	static void SetSpotCutoff(int lightIndex, float cutoffAngle) {
		// cutoffAngleを設定 (スポットライトの範囲角度)
		glLightf(GL_LIGHT0 + lightIndex, GL_SPOT_CUTOFF, cutoffAngle);
	}

	// スポットライトの中心部の明るさ設定
	static void SetSpotExponent(int lightIndex, float exponent) {
		// exponentを設定（明るさの集中度）
