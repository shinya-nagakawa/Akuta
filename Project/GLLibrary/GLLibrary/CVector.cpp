...
//新しく追加した
CVector3D CVector3D::ToRotation(const CVector3D& forward) const
{
	CVector3D rot;

	// 法線ベクトルからピッチ (x軸回り) を計算
	rot.x = atan2(this->y, sqrt(this->x * this->x + this->z * this->z));

	// 前方向ベクトルからヨー (y軸回り) を計算
	rot.y = atan2(forward.x, forward.z);

	// ロール (z軸回り) を計算
	rot.z = atan2(-this->x, this->y);
...
CQuaternion CQuaternion::Euler(float ax, float ay, float az)
{
	CQuaternion q;
	q.SetEuler(ax, ay, az);
	return q;
}
CQuaternion CQuaternion::Euler(const CVector3D& e)
{
